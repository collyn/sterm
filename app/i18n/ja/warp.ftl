...
agent-management-source-local-agent = STerm (ローカルエージェント)
agent-management-source-cloud-agent = STerm (ローカルエージェント)
...
workspace-menu-update-warp-manually = STerm を手動で更新
...
workspace-menu-view-warp-logs = STerm ログを表示
...
workspace-update-and-relaunch-warp = STerm を更新して再起動
workspace-updating-to-version = ({ $version }) に更新中
workspace-update-warp-manually = STerm を手動で更新
...
terminal-banner-shell-config-incompatible = シェルの設定が STerm と互換性がありません...{"  "}
...
ai-document-save-and-sync-tooltip = この計画を Drive に保存して自動同期
ai-document-show-in-warp-drive = Drive で表示
...
ai-footer-enable-notifications-tooltip = Warp プラグインをインストールして STerm 内のリッチなエージェント通知を有効化
...
sterm-launch-visit-repo = リポジトリを見る
sterm-launch-title = STerm がオープンソースになりました
sterm-launch-description = コミュニティの皆さんがエージェントファーストのワークフローで STerm の構築に参加できます。
sterm-launch-contribute-title = コントリビュート
sterm-launch-contribute-description = STerm のクライアントコードがオープンソースになりました。/feedback スキルで Issue を作成し、こちらのコントリビューションガイドラインに従ってください。
sterm-launch-contribute-link-text = こちら
sterm-launch-oad-title = Open Automated Development
sterm-launch-oad-description = STerm リポジトリは、Oz のローカルエージェント体験を活用したエージェントファーストワークフローで管理されています。
sterm-launch-auto-model-title = 'auto (open-weights)' 登場
sterm-launch-auto-model-description = タスクに最適なオープンウェイトモデル (Kimi、MiniMax など) を選ぶ新しい auto モデルを追加しました。
...
terminal-warpify-subshell-tooltip = このセッションで STerm シェル統合を有効化
terminal-use-agent = エージェントを使う
terminal-use-agent-tooltip = STerm エージェントに支援を依頼
terminal-give-control-back-to-agent = 制御をエージェントに戻す
terminal-resume-agent-tooltip = STerm エージェントに再開を依頼
...
terminal-open-in-warp = STerm で開く
terminal-open-conversation-in-warp-tooltip = この会話を STerm デスクトップアプリで開く
...
terminal-input-agent-hint-deploy-react-vercel = 何でも STerm で。例: React アプリを Vercel にデプロイし環境変数を設定
terminal-input-agent-hint-debug-python-ci = 何でも STerm で。例: CI で Python テストが失敗する原因をデバッグ
terminal-input-agent-hint-setup-microservice = 何でも STerm で。例: Docker で新しいマイクロサービスをセットアップしデプロイパイプラインを作成
terminal-input-agent-hint-fix-node-memory-leak = 何でも STerm で。例: Node.js アプリのメモリリークを発見して修正
terminal-input-agent-hint-backup-postgres = 何でも STerm で。例: PostgreSQL DB のバックアップスクリプトを作成しスケジュール
terminal-input-agent-hint-migrate-mysql-postgres = 何でも STerm で。例: MySQL から PostgreSQL へのデータ移行
terminal-input-agent-hint-monitor-aws = 何でも STerm で。例: AWS インフラの監視とアラートを設定
terminal-input-agent-hint-build-fastapi = 何でも STerm で。例: FastAPI でモバイルアプリ向け REST API を構築
terminal-input-agent-hint-optimize-sql = 何でも STerm で。例: 遅い SQL クエリを最適化
terminal-input-agent-hint-github-actions = 何でも STerm で。例: マージ時に自動デプロイする GitHub Actions ワークフローを作成
terminal-input-agent-hint-redis-cache = 何でも STerm で。例: Web アプリに Redis キャッシュを設定
terminal-input-agent-hint-kubernetes-pods = 何でも STerm で。例: Kubernetes Pod がクラッシュし続ける原因をトラブルシュート
terminal-input-agent-hint-bigquery-pipeline = 何でも STerm で。例: CSV を処理して BigQuery に投入するデータパイプラインを構築
terminal-input-agent-hint-ssl-https = 何でも STerm で。例: SSL 証明書を設定してドメインを HTTPS 化
terminal-input-agent-hint-refactor-legacy-code = 何でも STerm で。例: レガシーコードをモダンな設計パターンにリファクタ
terminal-input-agent-hint-unit-tests = 何でも STerm で。例: 認証サービスのユニットテストを作成
terminal-input-agent-hint-elk-logs = 何でも STerm で。例: 分散システム向けに ELK スタックでログ集約を構築
terminal-input-agent-hint-oauth-express = 何でも STerm で。例: Express.js アプリに OAuth2 認証を実装
terminal-input-agent-hint-optimize-docker = 何でも STerm で。例: ビルド時間とサイズを削減するため Docker イメージを最適化
terminal-input-agent-hint-ab-testing = 何でも STerm で。例: Web アプリ向けに A/B テスト基盤を構築
...
terminal-model-specs-description = STerm のハーネスでのモデル性能、クレジット消費レート、タスク速度のベンチマーク。
...
onboarding-callout-meet-input-title = STerm 入力欄のご紹介
...
onboarding-callout-nl-support-text-prefix = 自然言語入力は既定で無効です。有効にすると、平易な英語でリクエストを入力でき、STerm がエージェント向けクエリを自動判定します。次の方法でいつでも上書きできます:
onboarding-callout-enable-nl-detection = 自然言語判定を有効にする
onboarding-callout-new-agent-title = STerm の新しいエージェント体験のご紹介
...
language-widget-secondary = 変更を完全に反映するには STerm を再起動してください。
language-restart-required-title = 言語が変更されました
language-restart-required-body = STerm の UI 言語が更新されました。一部のテキストは即座に切り替わりますが、すべての箇所に反映するには再起動が必要です。
# =============================================================================
...
# STerm: settings-section-account は Account 設定ページとともに削除。
...
settings-section-warp-agent = STerm エージェント
...
settings-about-automatic-updates-description = 有効にすると、STerm はバックグラウンドで新バージョンを確認しダウンロードします。無効でも手動で更新を確認できます。
...
# STerm: main_page.rs 関連の文字列は Account 設定ページとともに削除された。
...
# ---- list_page.rs ----
settings-mcp-list-description = MCP サーバーを追加して STerm エージェントの機能を拡張します。MCP サーバーは標準化されたインターフェースを通じてデータソースやツールをエージェントに公開し、プラグインのように動作します。カスタムサーバーを追加するか、プリセットを使用して人気のサーバーから始められます。
...
settings-mcp-list-section-shared-by-warp-and-team = STerm と { $name } から利用可能
settings-mcp-list-section-shared-by-warp-and-other-devices = STerm および別のデバイスから共有
settings-mcp-list-section-shared-from-warp = STerm から共有
...
settings-platform-description = API キーを作成・管理して、ローカルエージェントから STerm アカウントへのアクセスを許可します。
    詳細は次を参照
...
settings-platform-zero-state-description = キーを作成して STerm への外部アクセスを管理します
settings-platform-create-api-key-description-personal = この API キーはユーザーに紐付き、STerm アカウントに対するリクエストを実行できます。
settings-platform-create-api-key-description-team = この API キーはチームに紐付き、チームを代理してリクエストを実行できます。
settings-platform-create-api-key-name-placeholder = STerm API キー
...
settings-referrals-page-title = 友人を STerm に招待
settings-referrals-anonymous-header = ローカル STerm ビルドでは紹介プログラムを利用できません
...
settings-referrals-reward-intro = 紹介で STerm 限定グッズを獲得*
...
settings-warpify-description-prefix = STerm が特定のシェルを「Warpify」(ブロック・入力モード等のサポートを追加) するかを設定します。
...
settings-warpify-install-ssh-extension-description = リモートホストに STerm の SSH 拡張がインストールされていない場合のインストール挙動を制御します。
...
# Section / sub-headers
settings-ai-warp-agent-header = STerm エージェント
...
settings-ai-command-denylist-description = STerm エージェントが実行する前に必ず権限を確認するコマンドにマッチする正規表現。
settings-ai-command-allowlist = コマンド許可リスト
settings-ai-command-allowlist-description = STerm エージェントが自動的に実行できるコマンドにマッチする正規表現。
...
settings-ai-mcp-allowlist-description = STerm エージェントによるこれらの MCP サーバーの呼び出しを許可します。
settings-ai-mcp-denylist = MCP 拒否リスト
settings-ai-mcp-denylist-description = STerm エージェントはこのリストにある MCP サーバーを呼び出す前に必ず権限を確認します。
settings-ai-info-banner-managed-by-workspace = 一部の権限はワークスペースによって管理されています。

# Models / Profiles
settings-ai-base-model = ベースモデル
settings-ai-base-model-description = このモデルは STerm エージェントの中心エンジンとして機能します。ほとんどのやり取りを担い、必要に応じて計画やコード生成などのタスクで他のモデルを呼び出します。STerm はモデルの可用性や、会話要約などの補助タスクに応じて自動的に別のモデルへ切り替えることがあります。
settings-ai-show-model-picker-in-prompt = プロンプトにモデル選択を表示
settings-ai-codebase-context = コードベースのコンテキスト
settings-ai-codebase-context-description = STerm エージェントによるコードベースの概要生成を許可し、コンテキストとして利用します。コードがサーバーに保存されることはありません。
...
settings-ai-rules-description = ルールは、STerm エージェントがコードベースや特定のワークフローのお作法に沿うよう導きます。
...
settings-ai-cloud-agent-computer-use-description = STerm アプリから開始したエージェント会話でコンピュータ操作を有効化します。

# AWS Bedrock
settings-ai-aws-bedrock-toggle = AWS Bedrock の認証情報を使用
settings-ai-aws-bedrock-description = STerm はローカルの AWS CLI 認証情報を読み込み、Bedrock 対応モデルへ送信します。
settings-ai-aws-bedrock-description-managed = STerm はローカルの AWS CLI 認証情報を読み込み、Bedrock 対応モデルへ送信します。この設定は組織によって管理されています。
...
settings-features-ssh-wrapper = STerm SSH ラッパー
settings-features-ssh-auto-discovery = SSH ホストの自動検出
settings-features-receive-desktop-notifications = STerm からのデスクトップ通知を受信
...
settings-features-make-default-terminal = STerm を既定のターミナルにする
...
settings-features-default-terminal-current = STerm が既定のターミナルです
...
settings-features-restart-warp-to-apply = 変更を反映するには STerm を再起動してください。
...
settings-code-folder-colors = フォルダカラー
settings-code-folder-colors-desc = プロジェクトエクスプローラーでフォルダごとに異なる色で描画します。
...
settings-exec-profile-editor-base-model-desc = このモデルはエージェントの主要エンジンとして機能します。ほとんどのやり取りを駆動し、必要に応じて計画やコード生成などのために他モデルを呼び出します。STerm はモデルの可用性や、会話の要約などの補助タスクのため、自動的に代替モデルに切り替える場合があります。
...
settings-show-blocks-confirm-dialog-text = このブロックを共有解除してもよろしいですか?
    ...
    リンク経由でアクセスできなくなり、STerm サーバーから完全に削除されます。
...
settings-appearance-custom-icon-restart-warning = MacOS で希望のアイコンスタイルを適用するには STerm の再起動が必要な場合があります。
...
settings-appearance-input-mode-pinned-bottom = 下部に固定 (STerm モード)
...
settings-environments-page-description = 環境はアンビエントエージェントが実行される場所を定義します。GitHub (推奨)、STerm 支援セットアップ、または手動構成で数分で設定できます。
...
settings-environments-section-team-default = STerm とこのデバイスから提供
settings-environments-section-team-named = STerm と { $team } で共有
...
quit-warning-title-app = STerm を終了しますか?
...
keybinding-desc-workspace-left-panel-warp-drive = 左パネル: Drive
...
keybinding-desc-workspace-toggle-warp-drive = Drive を切り替え
...
keybinding-desc-workspace-search-drive = Drive を検索
...
keybinding-desc-workspace-terminate-app = STerm を終了
...
keybinding-desc-workspace-export-all-warp-drive-objects = すべての Drive オブジェクトをエクスポート
keybinding-desc-workspace-install-cli = Oz CLI コマンドをインストール
keybinding-desc-workspace-uninstall-cli = Oz CLI コマンドをアンインストール

# AI assistant / agents
keybinding-desc-workspace-toggle-ai-assistant = STerm AI を切り替え
...
# STerm: keybinding-desc-workspace-show-settings-account は Account 設定ページとともに削除。
...
keybinding-desc-workspace-show-settings-about-menu = STerm について
...
keybinding-desc-workspace-view-logs = STerm ログを表示
...
keybinding-desc-workspace-open-sterm-launch-modal = [Debug] STerm Launch モーダルを開く
keybinding-desc-workspace-reset-sterm-launch-modal-state = [Debug] STerm Launch モーダル状態をリセット
...
keybinding-desc-input-show-network-log = STerm ネットワークログを表示
...
keybinding-desc-ai-assistant-close = STerm AI を閉じる
keybinding-desc-ai-assistant-focus-terminal-input = STerm AI からターミナル入力にフォーカス
keybinding-desc-ai-assistant-restart = STerm AI を再起動
...
keybinding-desc-terminal-ask-ai-selection = 選択について STerm AI に質問
keybinding-desc-terminal-ask-ai-last-block = 直前のブロックについて STerm AI に質問
keybinding-desc-terminal-ask-ai = STerm AI に質問
...
toggle-suffix-ssh-wrapper = STerm SSH ラッパー
...
settings-external-editor-choose-layout = STerm でファイルを開くレイアウトを選択
...
settings-external-editor-prefer-markdown = Markdown ファイルを既定で STerm の Markdown ビューアで開く
...
menu-block-open-in-warp = STerm で開く
...
menu-block-ask-warp-ai = STerm AI に質問
...
menu-input-ask-warp-ai = STerm AI に質問
...
prompt-editor-warp-prompt-section = STerm ターミナルプロンプト
...
welcome-tips-command-palette-description = キーボードから手を離さずに STerm の機能をすべて簡単に発見できます。
...
welcome-tips-theme-picker-description = 組み込みテーマから選んで STerm を自分好みに。または自作も可能。
...
drive-local-storage-limit-description = ローカルストレージ上限はこのデバイス上で適用されます。不要な項目を削除して、新しい Drive オブジェクト用の容量を空けてください。
...
workflow-ai-assist-tooltip = STerm AI でタイトル・説明・パラメータを生成
...
wasm-nux-open-desktop-title = STerm デスクトップで開きますか?
wasm-nux-open-desktop-detail = 今後のリンクは自動的にデスクトップで開きます。
wasm-nux-open-desktop-confirm = STerm で開く
wasm-nux-download-title = STerm デスクトップをダウンロードしますか?
wasm-nux-download-description = STerm は AI と開発チームのナレッジを内蔵したインテリジェントなターミナルです。
wasm-nux-learn-more = 詳しく見る
wasm-nux-download-confirm = ダウンロード
wasm-nux-object-kind-drive-objects = Drive オブジェクト
wasm-nux-object-kind-warp-links = STerm リンク
...
auth-override-warning-confirm-title = 個人の Drive オブジェクトと環境設定を削除しますか?
auth-override-warning-description = Web ブラウザから STerm アカウントにログインしたようです。続行すると、この匿名セッションの個人 Drive オブジェクトおよび環境設定はすべて完全に削除されます。
...
auth-override-warning-accessibility-help = STerm が Web ブラウザからの新しいログインを検出しました。ログインせずに STerm の使用を続けるには Esc を押してキャンセルしてください。
...
auth-needs-sso-link-detail = 下のボタンをクリックして STerm アカウントを SSO プロバイダにリンクしてください。
...
auth-offline-first-use-description = 現在オフラインです。STerm を初めて使用するにはインターネット接続が必要です。
auth-offline-first-use-learn-more = 詳しく見る
auth-offline-overlay-title = STerm をオフラインで使用
auth-offline-overlay-paragraph-1 = STerm のローカル機能はオフラインで動作します。
auth-offline-overlay-paragraph-2 = BYOP AI 機能を使う場合のみ、選択したプロバイダーへの接続が必要です。
auth-offline-overlay-paragraph-3 = STerm は匿名のローカルユーザー ID で動作し、外部オブジェクトや利用量計測のためにインターネット接続を要求しません。
auth-offline-overlay-dismiss = 閉じる
auth-privacy-settings-title = プライバシー設定
auth-privacy-settings-done = 完了
auth-privacy-settings-help-improve = STerm の改善に協力する
auth-privacy-settings-help-improve-description = ハイレベルな機能利用データは STerm プロダクトチームのロードマップ優先順位付けに役立ちます。
auth-privacy-settings-learn-more = 詳しく見る
auth-privacy-settings-send-crash-reports = クラッシュレポートを送信
auth-privacy-settings-crash-reports-description = クラッシュレポートは STerm エンジニアリングチームが安定性を理解しパフォーマンスを改善するのに役立ちます。
...
auth-logout-unsynced-drive-objects-warning = 未同期の Drive オブジェクトが { $count } { $count ->
        [one] 件
       *[other] 件
    }あります。ログアウトすると、{ $count ->
        [one] そのオブジェクト
...
cli-agent-plugin-codex-install-title = Codex の STerm 通知を有効化
cli-agent-plugin-codex-install-subtitle = Codex を最新版に更新し、フォーカス時通知を有効にすることで STerm が作業中に通知を表示できるようにします。
...
cli-agent-plugin-deepseek-install-title = DeepSeek の STerm 通知を有効化
...
terminal-bootstrapping-installing-warp-ssh-extension-progress = STerm SSH 拡張をインストール中... ({ $p }%)
terminal-bootstrapping-installing-warp-ssh-extension = STerm SSH 拡張をインストール中...
terminal-bootstrapping-updating-warp-ssh-extension = STerm SSH 拡張を更新中...
...
agent-tip-command-palette = <keybinding> でコマンドパレットを開き、STerm のアクションやショートカットにアクセスできます。
...
agent-tip-add-context = `@` でファイル、ブロック、Drive オブジェクトからコンテキストをプロンプトに追加できます。
...
hoa-welcome-banner-title = ユニバーサルエージェントサポートのご紹介: STerm であらゆるコーディングエージェントをレベルアップ
...
hoa-feature-native-code-review-description = STerm のコードレビューからインラインコメントを Claude Code、Codex、OpenCode へ直接送信
resource-center-whats-new-section = 新着情報
resource-center-getting-started-section = はじめに
resource-center-maximize-warp-section = STerm を最大限に活用
...
resource-center-command-palette-description = キーボードから STerm のすべての機能にアクセス。
resource-center-set-theme-title = テーマを設定
resource-center-set-theme-description = テーマを選んで STerm を自分好みに。
resource-center-custom-prompt-title = カスタムプロンプトを使用
resource-center-custom-prompt-description = PS1 設定を尊重するよう STerm をセットアップ
resource-center-view-documentation = ドキュメントを表示
resource-center-integrate-ide-title = STerm を IDE と連携
resource-center-integrate-ide-description = よく使う開発ツールから STerm を起動できるよう設定
resource-center-how-warp-uses-warp-title = STerm チームの STerm 活用法
resource-center-how-warp-uses-warp-description = STerm のエンジニアリングチームがお気に入り機能をどう使っているか学びます
...
onboarding-intention-title = STerm へようこそ
...
onboarding-ai-feature-warp-agents = STerm エージェント
...
onboarding-agent-title = STerm エージェントをカスタマイズ
...
onboarding-agent-disable-warp-agent = STerm エージェントを無効化
...
onboarding-project-subtitle = STerm でのコーディング向けにプロジェクトを設定します。
...
onboarding-customize-title = STerm をカスタマイズ
...
onboarding-free-user-agent-title = STerm 内蔵エージェントによるエージェント駆動開発
onboarding-free-user-agent-description = STerm 内蔵エージェント Oz でローカルに反復・計画・構築。
...
onboarding-free-user-subscribe-title = サブスクライブして STerm のエージェント駆動開発を利用しましょう。
...
onboarding-free-user-subscribe-item-drive = 無制限の Drive オブジェクトとコラボレーション
onboarding-free-user-subscribe-item-support = プライベートメールサポート
onboarding-free-user-subscribe-item-cloud-storage = ローカル会話保存

auth-opt-out-line-1 = STerm はオンボーディングの選択をこのデバイスに保存します。
...
auth-local-privacy-note = STerm はオンボーディングの選択をこのデバイスに保存します。
...
auth-login-slide-title-warp-drive = Drive をはじめる
...
auth-disable-warp-drive = Drive を無効化
auth-disable-ai-features = AI 機能を無効化
auth-enable-warp-drive = Drive を有効化
...
auth-disable-warp-drive-confirm-title = 本当に Drive を無効化しますか？
auth-disable-ai-features-confirm-title = 本当に AI 機能を無効化しますか？
auth-disable-warp-drive-confirm-body = Drive はワークフローやナレッジをデバイス間で保存し、チームと共有できます。続行すると、以下の機能が利用できなくなります:
auth-disable-ai-features-confirm-body = STerm は AI でより便利になります。続行すると、以下の機能はいずれも利用できなくなります:
...
auth-require-login-ai-collaboration = ローカル AI 機能に STerm アカウントは必要ありません。
auth-require-login-drive-limit = STerm では Drive オブジェクトをローカルに保存します。
auth-require-login-share = ローカル STerm ビルドでは共有機能を利用できません。
auth-welcome-title = STerm へようこそ！
auth-sign-up-for-warp = STerm を続行
...
get-started-welcome-title = STerm へようこそ
...
terminal-banner-p10k-supported = Powerlevel10k が STerm に対応しました！{"  "}
...
terminal-banner-pure-unsupported = Pure はまだ STerm で対応していません。代替として対応プロンプトの利用をご検討ください。{"  "}
...
settings-import-reset-to-warp-defaults = STerm の既定にリセット
...
workspace-notification-permission-denied-toast = STerm にはデスクトップ通知を送る権限がありません。
workspace-troubleshoot-notifications-link = 通知のトラブルシューティング
workspace-plan-synced-to-warp-drive-toast = プランを Drive に同期しました
workspace-remote-control-link-copied-toast = リモートコントロールリンクをコピーしました。
workspace-update-now = 今すぐ更新
workspace-update-warp = STerm を更新
...
workspace-version-deprecation-without-permissions-banner = 直ちに更新しないと一部の STerm 機能が想定どおり動作しない可能性がありますが、STerm は更新を実行できません。
workspace-new-version-unable-to-update-banner = 新しいバージョンが利用可能ですが、STerm は更新を実行できません。
workspace-unable-to-launch-new-installed-version = STerm はインストール済みの新バージョンを起動できませんでした。
...
terminal-ssh-report-issue-prefix = STerm の SSH 安定性向上に取り組んでいます。次の対応をご検討ください:{" "}
...
terminal-agent-mode-setup-title = このコードベース向けに STerm を最適化しますか？
...
notifications-banner-permissions-denied-title = STerm は通知送信の権限を拒否されました。
...
tooltip-secrets-not-sent-to-warp-server = *シークレットは STerm のサーバーに送信されません。
