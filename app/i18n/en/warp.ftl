# STerm Desktop — English (source-of-truth locale)
...
common-skip = Skip
common-get-warping = Get STermping
...
agent-management-source-local-agent = STerm (local agent)
agent-management-source-cloud-agent = STerm agent
...
workspace-menu-update-warp-manually = Update STerm manually
...
workspace-menu-view-warp-logs = View STerm logs
...
app-menu-set-default-terminal = Set STerm as Default Terminal
app-menu-file = File
app-menu-edit = Edit
app-menu-use-warp-prompt = Use STerm's Prompt
...
app-menu-help = Help
app-menu-warp-documentation = STerm Documentation...
app-menu-github-issues = GitHub Issues...
app-menu-warp-slack-community = STerm Slack Community...
workspace-update-and-relaunch-warp = Update and relaunch STerm
workspace-updating-to-version = Updating to ({ $version })
workspace-update-warp-manually = Update STerm manually
...
terminal-banner-shell-config-incompatible = Your shell configuration is incompatible with STerm...{"  "}
...
ssh-remote-choice-install-extension = Install STerm's SSH extension
ssh-remote-choice-install-extension-desc = Install STerm's extension to enable agent features like file browsing, code review, and intelligent command completions in this session.
...
ai-document-save-and-sync-tooltip = Save and auto-sync this plan to your Drive
ai-document-show-in-warp-drive = Show in Drive
...
ai-footer-enable-notifications-tooltip = Install the Warp plugin to enable rich agent notifications within STerm
...
sterm-launch-visit-repo = Visit the repo
sterm-launch-title = STerm is now open-source
sterm-launch-description = You, our community, can participate in building STerm using an agent-first workflow.
sterm-launch-contribute-title = Contribute
sterm-launch-contribute-description = STerm's client code is now open source. Get started by using the /feedback skill to open an issue, and follow the contribution guidelines here.
sterm-launch-contribute-link-text = here
sterm-launch-oad-title = Open Automated Development
sterm-launch-oad-description = The STerm repo is managed by an agent-first local workflow powered by Oz.
sterm-launch-auto-model-title = Introducing 'auto (open-weights)'
sterm-launch-auto-model-description = We've added a new auto model that picks the best open weight model for a task, like Kimi or MiniMax.
hoa-see-whats-new = See what's new
hoa-finish = Finish
session-config-get-warping = Get STermping
...
terminal-warpify-subshell-tooltip = Enable STerm shell integration in this session
terminal-use-agent = Use agent
terminal-use-agent-tooltip = Ask the STerm agent to assist
terminal-give-control-back-to-agent = Give control back to agent
terminal-resume-agent-tooltip = Ask the STerm agent to resume
...
terminal-open-in-warp = Open in STerm
terminal-open-conversation-in-warp-tooltip = Open this conversation in the STerm desktop app
...
terminal-input-agent-hint-deploy-react-vercel = STerm anything e.g. Deploy my React app to Vercel and set up environment variables
terminal-input-agent-hint-debug-python-ci = STerm anything e.g. Help me debug why my Python tests are failing in CI
terminal-input-agent-hint-setup-microservice = STerm anything e.g. Set up a new microservice with Docker and create the deployment pipeline
terminal-input-agent-hint-fix-node-memory-leak = STerm anything e.g. Find and fix the memory leak in my Node.js application
terminal-input-agent-hint-backup-postgres = STerm anything e.g. Create a backup script for my PostgreSQL database and schedule it
terminal-input-agent-hint-migrate-mysql-postgres = STerm anything e.g. Help me migrate my data from MySQL to PostgreSQL
terminal-input-agent-hint-monitor-aws = STerm anything e.g. Set up monitoring and alerts for my AWS infrastructure
terminal-input-agent-hint-build-fastapi = STerm anything e.g. Build a REST API for my mobile app using FastAPI
terminal-input-agent-hint-optimize-sql = STerm anything e.g. Help me optimize my SQL queries that are running slowly
terminal-input-agent-hint-github-actions = STerm anything e.g. Create a GitHub Actions workflow to automatically deploy on merge
terminal-input-agent-hint-redis-cache = STerm anything e.g. Set up Redis caching for my web application
terminal-input-agent-hint-kubernetes-pods = STerm anything e.g. Help me troubleshoot why my Kubernetes pods keep crashing
terminal-input-agent-hint-bigquery-pipeline = STerm anything e.g. Build a data pipeline to process CSV files and load them into BigQuery
terminal-input-agent-hint-ssl-https = STerm anything e.g. Set up SSL certificates and configure HTTPS for my domain
terminal-input-agent-hint-refactor-legacy-code = STerm anything e.g. Help me refactor this legacy code to use modern design patterns
terminal-input-agent-hint-unit-tests = STerm anything e.g. Create unit tests for my authentication service
terminal-input-agent-hint-elk-logs = STerm anything e.g. Set up log aggregation with ELK stack for my distributed system
terminal-input-agent-hint-oauth-express = STerm anything e.g. Help me implement OAuth2 authentication in my Express.js app
terminal-input-agent-hint-optimize-docker = STerm anything e.g. Optimize my Docker images to reduce build times and size
terminal-input-agent-hint-ab-testing = STerm anything e.g. Set up A/B testing infrastructure for my web application
...
terminal-model-specs-description = STerm's benchmarks for how well a model performs in our harness, the rate at which it consumes credits, and task speed.
...
onboarding-callout-meet-input-title = Meet the STerm input
...
onboarding-callout-nl-support-text-prefix = Natural language input is off by default. If enabled, you can type requests in plain English and STerm will autodetect queries for the agent. You can always override them using
onboarding-callout-enable-nl-detection = Enable Natural Language Detection
onboarding-callout-new-agent-title = Introducing STerm's new agent experience
...
language-widget-secondary = Restart STerm for the change to fully take effect.
language-restart-required-title = Language changed
language-restart-required-body = STerm's UI language has been updated. Some text will switch immediately, but a full restart is required for the change to take effect everywhere.
...
# STerm: settings-section-account removed alongside the Account settings page.
...
# about_page.rs
settings-about-copyright = Copyright 2026 STerm
settings-about-automatic-updates-label = Automatic updates
settings-about-automatic-updates-description = When enabled, STerm checks for new versions in the background and downloads the installer to a local cache. The currently running STerm is not touched until you click "Install now" to launch the installer yourself.
settings-about-update-checking = Checking for updates…
settings-about-update-up-to-date = STerm is up to date.
...
settings-about-update-install-hint-macos = The installer will open — drag STerm into your Applications folder to finish.
settings-about-update-install-hint-windows = The setup wizard will launch — follow the prompts to finish the upgrade.
settings-about-update-install-hint-linux = The AppImage will be replaced in place and STerm will restart.
...
# STerm: main_page.rs (Account / version / autoupdate) strings removed alongside
...
# ---- list_page.rs ----
settings-mcp-list-description = Add MCP servers to extend the STerm Agent's capabilities. MCP servers expose data sources or tools to agents through a standardized interface, essentially acting like plugins. Add a custom server, or use the presets to get started with popular servers.
...
settings-mcp-list-section-shared-by-warp-and-team = Available from STerm and { $name }
settings-mcp-list-section-shared-by-warp-and-other-devices = Shared by STerm and from other devices
settings-mcp-list-section-shared-from-warp = Shared from STerm
...
settings-platform-description = Create and manage API keys to allow local agents to access your STerm account.
...
settings-platform-zero-state-description = Create a key to manage external access to STerm
settings-platform-create-api-key-description-personal = This API key is tied to your user and can make requests against your STerm account.
...
settings-platform-create-api-key-name-placeholder = STerm API Key
...
settings-referrals-page-title = Invite a friend to STerm
settings-referrals-anonymous-header = Referral program is unavailable in local STerm builds
...
settings-referrals-reward-intro = Get exclusive STerm goodies when you refer someone*
...
settings-warpify-description-prefix = Configure whether STerm attempts to "Warpify" (add support for blocks, input modes, etc) certain shells.
...
settings-warpify-install-ssh-extension-description = Controls the installation behavior for STerm's SSH extension when a remote host doesn't have it installed.
...
# Section / sub-headers
settings-ai-warp-agent-header = STerm Agent
...
settings-ai-command-denylist-description = Regular expressions to match commands that the STerm Agent should always ask permission to execute.
settings-ai-command-allowlist = Command allowlist
settings-ai-command-allowlist-description = Regular expressions to match commands that can be automatically executed by the STerm Agent.
...
settings-ai-mcp-allowlist-description = Allow the STerm Agent to call these MCP servers.
settings-ai-mcp-denylist = MCP denylist
settings-ai-mcp-denylist-description = The STerm Agent will always ask for permission before calling any MCP servers on this list.
...
settings-ai-base-model-description = This model serves as the primary engine behind the STerm Agent. It powers most interactions and invokes other models for tasks like planning or code generation when necessary. STerm may automatically switch to alternate models based on model availability or for auxiliary tasks such as conversation summarization.
settings-ai-show-model-picker-in-prompt = Show model picker in prompt
settings-ai-codebase-context = Codebase Context
settings-ai-codebase-context-description = Allow the STerm Agent to generate an outline of your codebase that can be used for context. No code is ever stored on our servers.
...
settings-ai-rules-description = Rules help the STerm Agent follow your conventions, whether for codebases or specific workflows.
...
settings-ai-cloud-agent-computer-use-description = Enable computer use in agent conversations started from the STerm app.

# AWS Bedrock
settings-ai-aws-bedrock-toggle = Use AWS Bedrock credentials
settings-ai-aws-bedrock-description = STerm loads and sends local AWS CLI credentials for Bedrock-supported models.
settings-ai-aws-bedrock-description-managed = STerm loads and sends local AWS CLI credentials for Bedrock-supported models. This setting is managed by your organization.
...
settings-features-ssh-wrapper = STerm SSH Wrapper
settings-features-ssh-auto-discovery = Auto-discover SSH hosts
settings-features-receive-desktop-notifications = Receive desktop notifications from STerm
...
settings-features-make-default-terminal = Make STerm the default terminal
...
settings-features-default-terminal-current = STerm is the default terminal
...
settings-features-restart-warp-to-apply = Restart STerm for changes to take effect.
...
settings-code-folder-colors = Folder colors
settings-code-folder-colors-desc = Render different folders with distinct colors in the project explorer.
...
settings-exec-profile-editor-base-model-desc = This model serves as the primary engine behind the agent. It powers most interactions and invokes other models for tasks like planning or code generation when necessary. STerm may automatically switch to alternate models based on model availability or for auxiliary tasks such as conversation summarization.
...
settings-show-blocks-confirm-dialog-text = Are you sure you want to unshare this block?
    ...
    It will no longer be accessible by link and will be permanently deleted from STerm servers.
...
settings-appearance-custom-icon-restart-warning = You may need to restart STerm for MacOS to apply the preferred icon style.
...
settings-appearance-input-type-warp = STerm
settings-appearance-input-type-shell = Shell (PS1)
settings-appearance-input-position-label = Input position
settings-appearance-input-mode-pinned-bottom = Pin to the bottom (STerm mode)
...
settings-environments-page-description = Environments define where your ambient agents run. Set one up in minutes via GitHub (recommended), STerm-assisted setup, or manual configuration.
...
settings-environments-section-team-default = Provided by STerm and this device
settings-environments-section-team-named = Shared by STerm and { $team }
...
quit-warning-title-app = Quit STerm?
...
# Manage Rules 页面(Drive 中的 AI Fact Collection)。
...
keybinding-desc-workspace-left-panel-warp-drive = Left Panel: Drive
...
keybinding-desc-workspace-toggle-warp-drive = Toggle Drive
keybinding-desc-workspace-toggle-warp-drive-menu = Drive
...
keybinding-desc-workspace-search-drive = Search Drive
...
keybinding-desc-workspace-terminate-app = Quit STerm
...
keybinding-desc-workspace-export-all-warp-drive-objects = Export all Drive objects
...
# AI assistant / agents
keybinding-desc-workspace-toggle-ai-assistant = Toggle STerm AI
...
# STerm: keybinding-desc-workspace-show-settings-account removed alongside the
...
keybinding-desc-workspace-show-settings-about-menu = About STerm
...
keybinding-desc-workspace-view-logs = View STerm logs
...
keybinding-desc-workspace-open-sterm-launch-modal = [Debug] Open STerm Launch Modal
keybinding-desc-workspace-reset-sterm-launch-modal-state = [Debug] Reset STerm Launch Modal State
...
keybinding-desc-input-show-network-log = Show STerm network log
...
keybinding-desc-ai-assistant-close = Close STerm AI
keybinding-desc-ai-assistant-focus-terminal-input = Focus Terminal Input From STerm AI
keybinding-desc-ai-assistant-restart = Restart STerm AI
...
keybinding-desc-terminal-ask-ai-selection = Ask STerm AI about Selection
keybinding-desc-terminal-ask-ai-last-block = Ask STerm AI about last block
keybinding-desc-terminal-ask-ai = Ask STerm AI
...
toggle-suffix-ssh-wrapper = STerm SSH wrapper
...
settings-external-editor-choose-layout = Choose a layout to open files in STerm
...
settings-external-editor-prefer-markdown = Open Markdown files in STerm's Markdown Viewer by default
...
menu-block-open-in-warp = Open in STerm
...
menu-block-ask-warp-ai = Ask STerm AI
...
menu-input-ask-warp-ai = Ask STerm AI
...
slash-cmd-open-skill-desc = Open a skill's markdown file in STerm's built-in editor
...
slash-cmd-open-file-desc = Open a file in STerm's code editor
...
prompt-editor-warp-prompt-section = STerm terminal prompt
...
welcome-tips-command-palette-description = Easily discover everything you can do in STerm without your hands leaving the keyboard.
...
welcome-tips-theme-picker-description = Make STerm your own by choosing a built-in theme. Or create your own.
...
search-filter-display-drive = Drive
...
drive-local-storage-limit-description = Local storage limits are enforced on this device. Remove unused items to create space for new Drive objects.
...
workflow-ai-assist-tooltip = Generate a title, descriptions, or parameters with STerm AI
...
workspace-left-panel-warp-drive = Drive
...
wasm-nux-open-desktop-title = Open in STerm Desktop?
wasm-nux-open-desktop-detail = Future links will automatically open on desktop.
wasm-nux-open-desktop-confirm = Open in STerm
wasm-nux-download-title = Download STerm Desktop?
wasm-nux-download-description = STerm is the intelligent terminal with AI and your dev team's knowledge built-in.
wasm-nux-learn-more = Learn more
wasm-nux-download-confirm = Download
wasm-nux-object-kind-drive-objects = Drive objects
wasm-nux-object-kind-warp-links = STerm links
...
auth-override-warning-confirm-title = Delete personal Drive objects and preferences?
auth-override-warning-description = It looks like you logged into a STerm account through a web browser. If you continue, any personal Drive objects and preferences from this anonymous session will be permanently deleted.
...
auth-override-warning-accessibility-help = STerm has detected a new login from a web browser. Press escape to cancel and continue using STerm without login.
...
auth-needs-sso-link-detail = Click the button below to link your STerm account to your SSO provider.
...
auth-offline-first-use-description = You are currently offline. An internet connection is required to use STerm for the first time.
auth-offline-first-use-learn-more = Learn more
auth-offline-overlay-title = Using STerm Offline
auth-offline-overlay-paragraph-1 = STerm can be used offline for local terminal and agent workflows.
...
auth-privacy-settings-help-improve = Help improve STerm
auth-privacy-settings-help-improve-description = High-level feature usage data helps STerm's product team prioritize the roadmap.
auth-privacy-settings-learn-more = Learn more
auth-privacy-settings-send-crash-reports = Send crash reports
auth-privacy-settings-crash-reports-description = Crash reporting helps STerm's engineering team understand stability and improve performance.
...
auth-logout-unsynced-drive-objects-warning = You have { $count } unsynced Drive { $count ->
        [one] object
       *[other] objects
...
cli-agent-plugin-codex-install-title = Enable STerm Notifications for Codex
cli-agent-plugin-codex-install-subtitle = Update Codex to the latest version, then enable in-focus notifications so STerm can display them while you work.
...
cli-agent-plugin-deepseek-install-title = Enable STerm Notifications for DeepSeek
...
cli-agent-plugin-claude-install-title = Install STerm Plugin for Claude Code
...
cli-agent-plugin-claude-update-title = Update STerm Plugin for Claude Code
...
cli-agent-plugin-gemini-install-title = Install STerm Plugin for Gemini CLI
...
cli-agent-plugin-gemini-update-title = Update STerm Plugin for Gemini CLI
...
cli-agent-plugin-opencode-install-title = Install STerm Plugin for OpenCode
...
cli-agent-plugin-opencode-update-title = Update STerm Plugin for OpenCode
...
terminal-bootstrapping-installing-warp-ssh-extension-progress = Installing STerm SSH Extension... ({ $p }%)
terminal-bootstrapping-installing-warp-ssh-extension = Installing STerm SSH Extension...
terminal-bootstrapping-updating-warp-ssh-extension = Updating STerm SSH Extension...
...
agent-tip-command-palette = <keybinding> to open the Command Palette and access STerm actions and shortcuts.
...
agent-tip-add-context = `@` to add context from files, blocks, or Drive objects to your prompt.
...
agent-tip-action-warp-drive = Drive.
...
hoa-welcome-banner-title = Introducing universal agent support: level up any coding agent with STerm
...
hoa-feature-native-code-review-description = Send inline comments from STerm's code review directly to Claude Code, Codex, or OpenCode
resource-center-whats-new-section = What's New?
resource-center-getting-started-section = Getting Started
resource-center-maximize-warp-section = Maximize STerm
...
resource-center-command-palette-description = Access all of STerm via the keyboard.
resource-center-set-theme-title = Set your theme
resource-center-set-theme-description = Make STerm your own by choosing a theme.
resource-center-custom-prompt-title = Use your custom prompt
resource-center-custom-prompt-description = Set up STerm to honor your PS1 setting
resource-center-view-documentation = View documentation
resource-center-integrate-ide-title = Integrate STerm with your IDE
resource-center-integrate-ide-description = Configure STerm to launch from your most used development tools
resource-center-how-warp-uses-warp-title = How STerm uses STerm
resource-center-how-warp-uses-warp-description = Learn how STerm's engineering team uses their favorite features
...
onboarding-intention-title = Welcome to STerm
...
onboarding-ai-feature-warp-agents = STerm agents
...
onboarding-agent-title = Customize your STerm Agent
...
onboarding-agent-disable-warp-agent = Disable STerm Agent
onboarding-project-title = Open a project
onboarding-project-subtitle = Set up a project to optimize it for coding in STerm.
...
onboarding-customize-title = Customize your STerm
...
onboarding-customize-warp-drive = Drive
onboarding-customize-tools-panel = Tools panel
onboarding-customize-code-review = Code review

auth-opt-out-line-1 = STerm stores onboarding choices locally.
...
auth-local-privacy-note = STerm stores onboarding choices locally on this device.
...
auth-login-slide-title-warp-drive = Get started with Drive
...
auth-disable-warp-drive = Disable Drive
auth-disable-ai-features = Disable AI features
auth-enable-warp-drive = Enable Drive
...
auth-disable-warp-drive-confirm-title = Are you sure you want to disable Drive?
auth-disable-ai-features-confirm-title = Are you sure you want to disable AI features?
auth-disable-warp-drive-confirm-body = Drive lets you save workflows and knowledge across devices and share them with your team. By continuing, you won't have access to the following features:
auth-disable-ai-features-confirm-body = STerm is better with AI. By continuing, you won't have access to any of the following features:
...
auth-require-login-ai-collaboration = Local AI features do not require a STerm account.
auth-require-login-drive-limit = Drive objects are stored locally in STerm.
auth-require-login-share = Sharing is unavailable in local STerm builds.
auth-welcome-title = Welcome to STerm!
auth-sign-up-for-warp = Continue in STerm
...
get-started-welcome-title = Welcome to STerm
...
terminal-banner-p10k-supported = Powerlevel10k now supports STerm!{"  "}
...
terminal-banner-pure-unsupported = Pure is not yet supported in STerm. You might consider one of the supported prompts as an alternative.{"  "}
...
settings-import-reset-to-warp-defaults = Reset to STerm defaults
...
workspace-notification-permission-denied-toast = STerm doesn't have permission to send desktop notifications.
workspace-troubleshoot-notifications-link = Troubleshoot notifications
workspace-plan-synced-to-warp-drive-toast = Plan synced to your Drive
workspace-remote-control-link-copied-toast = Remote control link copied.
workspace-update-now = Update now
workspace-update-warp = Update STerm
...
workspace-version-deprecation-without-permissions-banner = Some STerm features may not work as expected without updating immediately, but STerm is unable to perform the update.
workspace-new-version-unable-to-update-banner = A new version is available but STerm is unable to perform the update.
workspace-unable-to-launch-new-installed-version = STerm was unable to launch the new installed version.
...
terminal-ssh-report-issue-prefix = We are actively working on improving the stability of SSH in STerm. Please consider{" "}
...
terminal-agent-mode-setup-title = Optimize STerm for this codebase?
...
notifications-banner-permissions-denied-title = STerm was denied permissions to send you notifications.
...
tooltip-secrets-not-sent-to-warp-server = *Secrets are not sent to STerm's server.
