...
///
/// 取 `ChannelState::app_id().application_name()` 是为了让 OSS 构建显示 `STerm`、
/// 而 Stable/Preview/Dev 等上游 channel 仍显示各自的 `STerm` / `WarpPreview` / `WarpDev`,
/// 避免在 fork 中跨多处硬编码字符串(Windows 任务管理器按窗口标题做进程分组,
/// 硬编码 `"STerm"` 会让 STerm 在任务管理器里显示成 `STerm(N)`)。
///
