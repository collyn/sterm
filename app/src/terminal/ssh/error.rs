...
const SSH_GITHUB_ISSUE_URL: &str = "https://github.com/collyn/sterm/issues/new?assignees=&labels=Bugs,SSH-tmux&projects=&template=03_ssh_tmux.yml";

fn get_ssh_github_issue_url(title: &str) -> String {
    let url = if let Some(version) = ChannelState::app_version() {
        format!("{SSH_GITHUB_ISSUE_URL}&sterm-version={version}")
    } else {
