...
const INTERNAL_WARP_ERROR: &str = "Internal STerm error.";
...
fn render_mermaid_diagram_section<A: Action>(
    ...
    // Build header action buttons: copy source and expand/lightbox
    let header_actions = {
        ...
        let code_mouse = MouseStateHandle::default();
        let code_icon = ConstrainedBox::new(
            Icon::Code1
                .to_warpui_icon(header_icon_color.into())
                .finish(),
        )
        .with_width(16.)
        .with_height(16.)
        .finish();
        let code_btn = icon_button(appearance, Icon::Code1, false, code_mouse)
            .build()
            .finish();
        let _ = code_icon;
...
fn render_visual_card(
    ...
    header_actions: Option<Box<dyn Element>>,
    app: &AppContext,
) -> Box<dyn Element> {
    let appearance = Appearance::as_ref(app);
    let theme = appearance.theme();
