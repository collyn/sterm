...
impl FileTreeView {
    ...
    fn render_item_with_hover(
        ...
        let icon_color = if let Some(over) = render_state.icon_color_override {
            over
        } else {
            item_highlight_state.text_and_icon_color(appearance).into()
        };
        let icon = match render_state.icon {
            ImageOrIcon::Icon(icon) => icon.to_warpui_icon(icon_color).finish(),
    ...
    fn render_item_while_dragging(
        ...
        let render_state = item.to_render_state(None /* is_expanded */, app);
    ...
    fn render_item(&self, id: &FileTreeIdentifier, app: &AppContext) -> Box<dyn Element> {
        let Some(root_dir) = self.root_directories.get(&id.root) else {
            return Empty::new().finish();
        };
        let Some(item) = root_dir.items.get(id.index) else {
        ...
        let render_state = item.to_render_state(is_expanded, app);
        let appearance = Appearance::as_ref(app);
        ...
        let draggable = Draggable::new(draggable_state, hoverable)
            ...
            .with_alternate_drag_element(self.render_item_while_dragging(&id_for_drag, app))
            ...
            move |range: Range<usize>, app: &AppContext| {
                ...
                range
                    .filter_map(|global_index| {
                        ...
                        Some(view.render_item(&item_id, app))
                    })
                    .collect::<Vec<_>>()
                    .into_iter()
            },
