...
impl SettingsWidget for AboutPageWidget {
    ...
    fn search_terms(&self) -> &str {
        "about sterm version automatic updates auto update 自动更新 检查更新 新版本"
    }
    ...
    fn render(
        ...
        // 始终用纯图标 logo,品牌名以独立文本 "STerm" 呈现,不再依赖带 "warp" 字样的 svg
        let image_path = "bundled/png/sterm-logo.png";
        ...
        let (status_text, action) = match &stage {
            ...
            stage if stage.available_new_version().is_some() => {
                ...
                let url = github::cached_release()
                    .map(|r| r.html_url)
                    .unwrap_or_else(|| {
                        "https://github.com/collyn/sterm/releases/latest".to_owned()
                    });
