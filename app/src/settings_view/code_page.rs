    ...
    fn build_page(
        ...
        let (widgets, external_editor_view) = if FeatureFlag::ZapNewSettingsModes.is_enabled() {
            ...
            let widgets: Vec<Box<dyn SettingsWidget<View = Self>>> = vec![
                ...
                Box::new(FolderColorsToggleWidget::default()),
...
impl TypedActionView for CodeSettingsPageView {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            CodeSettingsPageAction::ToggleFolderColors => {
                CodeSettings::handle(ctx).update(ctx, |settings, ctx| {
                    report_if_error!(settings.enable_folder_colors.toggle_and_save_value(ctx));
                });
                ctx.notify();
            }
...
#[derive(Default)]
struct FolderColorsToggleWidget {
    switch_state: SwitchStateHandle,
}

impl SettingsWidget for FolderColorsToggleWidget {
    type View = CodeSettingsPageView;

    fn search_terms(&self) -> &str {
        "folder color colors project explorer directories"
    }
    ...
    fn render(
        ...
        render_body_item::<CodeSettingsPageAction>(
            crate::t!("settings-code-folder-colors"),
            None,
            LocalOnlyIconState::Hidden,
            ToggleState::Enabled,
            appearance,
            appearance
                .ui_builder()
                .switch(self.switch_state.clone())
                .check(*code_settings.enable_folder_colors)
                .build()
                .on_click(move |ctx, _, _| {
                    ctx.dispatch_typed_action(CodeSettingsPageAction::ToggleFolderColors);
                })
                .finish(),
            Some(crate::t!("settings-code-folder-colors-desc")),
        )
    }
}
