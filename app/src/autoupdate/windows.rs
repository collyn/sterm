    ...
    let url = if matches!(channel, Channel::Oss) {
        if let Some(release) = github::cached_release() {
            if let Some(found) = release.find_asset(&installer_file_name) {
                ...
                format!(
                    "https://github.com/collyn/sterm/releases/download/v{}/{installer_file_name}",
...
fn app_name_prefix(channel: Channel) -> &'static str {
    match channel {
        Channel::Stable => "STerm",
        Channel::Preview => "WarpPreview",
        Channel::Local => "warp",
        Channel::Integration => "integration",
        Channel::Dev => "WarpDev",
        // 与 script/windows/bundle.ps1 OSS 分支 INSTALLER_NAME=STerm+Setup 对齐,
        // 这样 GitHub Release 资产名 STermSetup.exe 能被 installer_file_name() 正确生成。
        Channel::Oss => "STerm",
    }
}
