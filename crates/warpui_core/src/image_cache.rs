...
static SVG_FONT_DB: LazyLock<Arc<usvg::fontdb::Database>> = LazyLock::new(|| {
    ...
    // Find a suitable fallback font family from the loaded system fonts
    let fallback_sans_serif = fontdb
        .faces()
    ...
    // Also do the same for serif and monospace if possible, or just default to same family
    let fallback_serif = fontdb
        .faces()
        .flat_map(|face| face.families.iter().map(|(family, _)| family.as_str()))
        .find(|family| {
            matches!(
    ...
    if let Some(family) = fallback_sans_serif.as_ref() {
        fontdb.set_sans_serif_family(family.as_str());
    }
    if let Some(family) = fallback_serif.as_ref().or(fallback_sans_serif.as_ref()) {
        fontdb.set_serif_family(family.as_str());
    }
    if let Some(family) = fallback_monospace.as_ref().or(fallback_sans_serif.as_ref()) {
        fontdb.set_monospace_family(family.as_str());
    }
