...
fn test_cache_dir_path() {
    ...
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            assert_eq!(cache_dir(), home_dir.join("Library/Application Support/dev.sterm.STerm"));
        } else if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
...
fn test_state_dir_path() {
    ...
    cfg_if::cfg_if! {
        // ChannelState, by default, is configured for Channel::Oss.
        if #[cfg(target_os = "macos")] {
            assert_eq!(state_dir(), home_dir.join("Library/Application Support/dev.sterm.STerm"));
        } else if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
...
fn test_project_path_for_zap_dev_app_id() {
    ...
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            assert_eq!(project_dirs.project_path(), "dev.sterm.STermDev");
        } else if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
...
fn test_project_path_for_oss_app_id() {
    ...
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            assert_eq!(project_dirs.project_path(), "dev.sterm.STerm");
        } else if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
