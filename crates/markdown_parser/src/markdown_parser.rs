...
fn parse_image_prefix_internal<'a, E: ContextError<&'a str> + ParseError<&'a str>>(
    ...
    alt((parse_markdown_image, parse_html_image))(input)
}

fn parse_markdown_image<'a, E: ContextError<&'a str> + ParseError<&'a str>>(
    input: &'a str,
) -> IResult<&'a str, FormattedImage, E> {
...
fn parse_html_image<'a, E: ContextError<&'a str> + ParseError<&'a str>>(
    input: &'a str,
) -> IResult<&'a str, FormattedImage, E> {
    let (input, _) = tag("<img")(input)?;
    let (input, attributes_str) = take_until(">")(input)?;
    let (input, _) = tag(">")(input)?;

    let src = extract_html_attribute(attributes_str, "src").unwrap_or_default();
    let alt_text = extract_html_attribute(attributes_str, "alt").unwrap_or_default();
    let title = extract_html_attribute(attributes_str, "title");
...
fn extract_html_attribute(attributes_str: &str, attr: &str) -> Option<String> {
    let mut current = attributes_str;
    while let Some(idx) = current.find(attr) {
        let rest = &current[idx + attr.len()..].trim_start();
        if rest.starts_with('=') {
...
fn parse_inline<'a, E: ContextError<&'a str> + ParseError<&'a str>>(
    ...
    while !input.is_empty() {
        ...
        match token {
            ...
            InlineToken::HtmlTag(tag) => {
                let lower = tag.to_lowercase();
                if lower.starts_with("<br") {
                    state.push_text("\n");
                }
                // Other tags parsed by parse_inline_html_tag are simply ignored (hidden)
            }
        }
    }
